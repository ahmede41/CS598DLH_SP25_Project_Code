    ...
    def __init__(self, x, mc_sample_size, window_size, eta=3, state=None, adf=False, acf=False, acf_plus=False, ACF_nghd_Threshold=0.4, ACF_out_nghd_Threshold=0.5):
        ...
        if not self.adf:
            ...
            for i in range(len(x)):
                acfs = []
                sample = x[i]
                print(f"⚠️ DEBUG: Sample[{i}] shape: {sample.shape}")
                
                if sample.ndim < 3:
                    raise ValueError(f"❌ Invalid sample shape at index {i}: expected ≥3D tensor (like [1, D, T]), got {sample.shape}")

                for f in range(sample.shape[-2]):
                    # Check that map (channel 1) exists and has enough values
                    if sample.shape[0] >= 2 and len(torch.where(sample[1, f, :] == 1)[0]) > sample.shape[-2] * 0.4:
                ...
                if len(acfs) > 0:
                    ...
                    self.acf_avgs.append(torch.zeros(sample.shape[-1]))
...
def main(train_encoder, data_type, encoder_type, encoder_hyper_params, learn_encoder_hyper_params, classification_hyper_params, cont, pretrain_hyper_params, plot_embeddings, unique_id, unique_name, DEBUG=False, circulatory_failure=False):
    ...
    if train_encoder:
        ...
        train_data = torch.load("../synthetic_hirid_dataset/train.pt")  # (N, 12, 6)
        test_data = torch.load("../synthetic_hirid_dataset/val.pt")     # (N, 12, 6)
        train_data = train_data.permute(0, 2, 1).unsqueeze(1)  # (N, 1, 6, 12)
        test_data = test_data.permute(0, 2, 1).unsqueeze(1)    # (N, 1, 6, 12)

        all_data = torch.cat([train_data, test_data], dim=0)
        learn_encoder(data_maps=all_data,
                      encoder_type=encoder_type, 
                      encoder_hyper_params=encoder_hyper_params,
                      pretrain_hyper_params=pretrain_hyper_params, 
                      **learn_encoder_hyper_params)
    ...
    for encoder_cv in range(learn_encoder_hyper_params['n_cross_val_encoder']):
        for classification_cv in range(classification_hyper_params['n_cross_val_classification']):
            ...
            if os.path.exists('../ckpt/%s/%s_checkpoint_%d.tar'%(data_type, UNIQUE_NAME, encoder_cv)): # i.e. a checkpoint *has* been saved for this config
                ...
                if not learn_encoder_hyper_params.get('ADF', False):
                    print('Levels of missingness, for Train, Validation, and TEST')
                    def safe_missingness_ratio(tensor, label):
                        if tensor.ndim == 4 and tensor.shape[1] > 1:
                            ratio = (tensor[:, 1, :, :] == 0).sum().item() / (tensor.shape[0] * tensor.shape[2] * tensor.shape[3])
                            print(f"{label} missingness ratio: {ratio:.4f}")
                        else:
                            print(f"⚠️ Skipping missingness stats for {label} — tensor shape is {tensor.shape}")

                    safe_missingness_ratio(train_mixed_data_maps_cv, "Train")
                    safe_missingness_ratio(validation_mixed_data_maps_cv, "Validation")
                    safe_missingness_ratio(TEST_mixed_data_maps, "TEST")               
                ...
                if os.path.exists('../ckpt/%s/%s_encoder_checkpoint_%d_%sClassifier_checkpoint_%d.tar'%(data_type, UNIQUE_ID, encoder_cv, 'circulatory' if circulatory_failure else '', classification_cv)):
                    ...
                    '''
                    if train_mixed_data_maps_cv.ndim < 2:
                        print("⚠️ Skipping linear classifier — input is not encoded features:", train_mixed_data_maps_cv.shape)
                        return
